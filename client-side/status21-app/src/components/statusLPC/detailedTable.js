...
export default function DetailedTable({ filter }) {
    ...
    useEffect(() => {
        async function fetchDetailedData() {
            ...
            try {
                ...
                // Extract data and dynamically determine headers from the first row
    ...
    const formatHeaderText = (header) => {
        ...
        if (header.includes('GMT+0800') || header.match(/\d{4}-\d{2}-\d{2}/) || header.match(/[A-Za-z]{3} [A-Za-z]{3} \d{2} \d{4}/)) {
            try {
